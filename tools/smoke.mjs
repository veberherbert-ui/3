...
await tp.evaluate(async () => {
  ...
  await put("days", [{ id: "d1", name: "Верх", exercises: ["Жим гантелей лёжа (горизонт)", "Тяга верхнего блока (V-хват)", "Фейспул", "Тяга гантели в наклоне одной рукой"] }]);
  ...
  await put("workouts", [
    day("w1", "2026-08-28", [
      ...
      { name: "Тяга гантели в наклоне одной рукой", uni: true, sets: [set(12, 30), set(19, 30), set(8, 30), set(6, 40)] },
    ]),
    day("w2", "2026-08-21", [
      { name: "Жим гантелей лёжа (горизонт)", pair: true, tags: ["pain"], sets: [set(12, 24), set(12, 24)] },
    ]),
...
/* Разовая проверка на тяжёлом весе не должна затыкать совет: подходы
   12, 19, 8 на рабочем весе — это тридцать девять повторений против
   тридцати шести нужных, восьмёрка тут долг за девятнадцать, а не предел. */
ok(/объём на рабочем весе выбран/.test(await hintOf("Тяга гантели в наклоне")),
  "объём на рабочем весе выбран — совет срабатывает, несмотря на провал в одном подходе");
