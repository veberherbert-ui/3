  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        /* Что не качать заранее.

           Предзагрузка — единственное место, где приложение решает за
           браузер. Обычно он умнее: в наших @font-face прописан unicode-range,
           и греческий кусок шрифта не запрашивается, пока на экране нет
           греческой буквы. А предзагрузка тянет всё подряд списком, и умение
           браузера пропадает впустую — на первой установке качалось 99 файлов
           шрифтов на 1,2 МБ, больше половины всего приложения.

           Поэтому в списке остаются только те начертания, которыми
           действительно что-то нарисуется. Проверено посимвольно: во всех
           текстах приложения нет ни одного знака вне latin и cyrillic —
           стрелки и значки меню (→ ⋮ ☰) не покрыты вообще ни одним куском
           этих шрифтов и всегда рисовались системным.

           Правило мягкое: файл не в списке не значит «недоступен». Если такой
           знак всё же понадобится — в названии дня, которое человек придумал
           сам, — браузер возьмёт его из сети, а без сети нарисует системным
           шрифтом. Ломаться тут нечему.

           woff отдельной строкой: это тот же шрифт в старом формате, дубль
           к woff2 для браузеров до Safari 10. Наша нижняя планка — 14.1. */
        globIgnores: [
          "**/screenshots/**",
          "**/*.woff",
          "**/*-greek-*",
