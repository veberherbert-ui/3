...
export function browserKind() {
  ...
  /* Safari на макбуке: тот же движок, но путь установки свой и совсем
     не тот, что у хрома, — значка в строке адреса там нет и не будет. */
  if (/Safari\//.test(s) && /Version\//.test(s)) return "safari-desktop";
  return "other";
}
...
/* Компьютер — отдельный случай, и у каждого браузера он свой.

   Chrome и Edge держат установку в строке адреса, Safari на маке —
   в меню «Файл», а Firefox не умеет ставить приложения вовсе, и честнее
   сказать это, чем гонять человека по меню в поисках несуществующего
   пункта. */
const DESKTOP = {
  "safari-desktop": [
    "Открой меню «Файл» в строке меню сверху",
    "Выбери «Добавить в Dock»",
    "Приложение появится в доке и будет открываться отдельным окном",
  ],
  firefox: [
    "Firefox на компьютере приложения не устанавливает — такой возможности у него нет",
    "Дневник полностью работает и во вкладке; чтобы не потерять, добавь в закладки",
    "Если нужен значок — открой эту ссылку в Chrome, Edge или Safari",
  ],
  other: [
...
export function installPlan() {
  ...
  const steps = phone
    ? STEPS[kind] || STEPS.other
    : kind === "inapp"
      ? STEPS.inapp
      : DESKTOP[kind] || DESKTOP.other;
  return {
    state: "manual",
